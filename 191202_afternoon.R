...
# convert digit labels to factor for classification(범주형으로)
...
test_labels<-testing[,1] #정답 집어넣기

#calculate number of correct prediction (시험결과랑 정답하고 비교해보기)
sum(diag(table(test_labels,pred.dl.df[,1])))
table(test_labels,pred.dl.df[,1])
diag(table(test_labels,pred.dl.df[,1]))
...
pred.dl<-h2o.predict(object=model.dl, newdata=tsData[,-2])
pred.dl.df<-as.data.frame(pred.dl)



summary(pred.dl,exact_quantiles=TRUE)
test_labels<-testing[,2] #정답 집어넣기

#calculate number of correct prediction (시험결과랑 정답하고 비교해보기)
sum(diag(table(test_labels,pred.dl.df[,2])))
table(test_labels,pred.dl.df[,2])
diag(table(test_labels,pred.dl.df[,2]))
